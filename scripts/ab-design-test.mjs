...
const requiredPrimary = new Set([
  "perks", "search", "groups", "scenes", "facilities", "locate", "tabs", "merged", "wizard",
]);
...
ok("index 引入 design-layouts", /design-layouts\.css/.test(fs.readFileSync(path.join(ROOT, "index.html"), "utf8")));

const modes = new Set(Object.values(variants).map((v) => v.cardMode));
ok("≥4 种卡片形态", modes.size >= 4);
const primaries = new Set(Object.values(variants).map((v) => v.filterPrimary));
ok("≥6 种主筛选交互", primaries.size >= 6);
...
${Object.values(variants)
  .map(
    (v) =>
      `| ${v.id} | ${v.name} | ${v.dimension} | ${v.filterPrimary} | ${v.cardMode} | ${v.description} |`
  )
  .join("\n")}
