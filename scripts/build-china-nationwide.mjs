...
const PARKING_LABELS = ["路侧免费泊位", "公共停车场", "市民中心停车场", "公园配套停车", "社区免费停车", "政务中心停车"];
const TOILET_LABELS = ["市民中心公厕", "公园公厕", "广场公厕", "滨河公厕", "市场公厕", "社区公厕"];
const CAMPING_LABELS = ["郊野露营点", "城市露营基地", "公园露营区", "湖畔露营点", "滨江露营点", "星空露营基地"];
const SPORTS_LABELS = ["全民健身中心", "公共体育馆", "社区运动场", "滨河体育公园", "青少年活动中心", "市民健身广场"];
const CHARGING_LABELS = ["公共充电站", "政务中心充电点", "公园充电站", "社区充电站", "公共快充站", "新能源充电站"];
const POLICY_LABELS = ["政务服务中心", "市民之家", "行政审批中心", "街道便民服务中心", "12345服务大厅", "跨省通办窗口"];
const LEGAL_LABELS = ["法律援助中心", "公共法律服务中心", "劳动仲裁咨询站", "消费维权站", "司法所", "公证咨询点"];
const TRAINING_LABELS = ["就业技能培训中心", "公共实训基地", "社区职培站", "青年职业技能站", "农民工培训中心", "创业培训站"];
/** 政务服务办事大厅（市民之家、各局窗口等） */
const GOV_SERVICE_LABELS = [
  "市民之家", "政务服务中心", "行政审批局", "人社局办事大厅", "劳动保障服务中心",
  "民政服务中心", "市场监督管理局", "公安局出入境办事大厅", "公安局户政办事大厅",
  "街道便民服务中心", "12345便民服务中心", "跨省通办专窗",
];
/** 法院及司法行政办事点位 */
const COURT_LABELS = [
  "区人民法院", "市中级人民法院", "基层人民法院", "劳动人事争议仲裁院", "行政复议咨询窗口", "检察服务中心",
];
/** 浙江地市真实区县（杭州加密用） */
const HZ_DISTRICTS = ["上城", "拱墅", "西湖", "滨江", "萧山", "余杭", "富阳", "临平", "钱塘", "临安", "桐庐", "淳安", "建德"];
...
function stableId(city, category, name) {
  const prefix = { library: "lib", reading: "read", museum: "mus", park: "park", tool: "tool", station: "sta", toilet: "toil", parking: "pkg", sports: "sport", community: "com", charging: "chg", bunker: "bnkr", policy: "pol", camping: "camp", legal: "legal", training: "train", gov_service: "gov", court: "court" }[category] || "res";
  let h = 0;
...
function mkAmenity(city, province, category, label, i, extra = {}) {
  const district = DISTRICT_SUFFIX[i % DISTRICT_SUFFIX.length];
  const defaults = {
    parking: {
      costType: /免费|路侧/.test(label) ? "free" : "conditional",
      hours: "以现场公示为准",
      facilities: { charge: "partial" },
      features: ["公共停车", label.includes("免费") ? "部分时段免费" : "便民停车"],
      note: "具体收费与开放以当地公告为准",
    },
    toilet: {
      costType: "free",
    ...
    camping: {
      ...
      facilities: {},
      features: ["露营", "户外"],
      note: "部分营地需预约，以当地公告为准",
      seasonal: true,
    },
    sports: {
      costType: "conditional",
      hours: "以场馆公告为准",
      facilities: { water: "partial", ac: "partial" },
      features: ["公共体育", "节假日部分免费"],
      note: "节假日部分场馆免费开放，以当地公告为准",
    },
    charging: {
      costType: "conditional",
      hours: "以站点公告为准",
      ...
      features: ["公共充电", "可充电"],
      note: "部分站点限时免费，以当地公告为准",
    },
    policy: {
      costType: "free",
      hours: "工作日 9:00—17:00",
      facilities: { water: "partial", ac: true },
      features: ["政务服务", "便民办事"],
      note: "具体业务以窗口公告为准",
    },
    legal: {
      costType: "free",
      hours: "工作日 9:00—17:00",
      facilities: { water: "partial" },
      features: ["法律咨询", "法律援助"],
      note: "咨询方式与范围以当地司法部门公告为准",
    },
    training: {
      costType: "free",
      hours: "以培训计划为准",
      facilities: { wifi: "partial", ac: true },
      features: ["职业培训", "技能提升"],
      note: "免费班次以人社部门公告为准",
    },
    gov_service: {
      costType: "free",
      hours: "工作日 9:00—17:00",
      facilities: { water: "partial", ac: true, wifi: "partial" },
      features: ["政务服务", "便民办事", "一窗受理"],
      note: "具体业务与窗口以当地政务部门公告为准",
    },
    court: {
      costType: "free",
      hours: "工作日 9:00—17:00",
      facilities: { water: "partial", ac: true },
      features: ["诉讼服务", "司法便民", "立案咨询"],
      note: "立案与咨询流程以法院公告为准",
    },
  };
  const d = defaults[category] || {};
  return {
    id: stableId(city, category, `${label}-${i}`),
    name: `${city}${district}${label}`,
    category,
    city,
    province,
    district,
    address: `${province}${city}${district}${label}`,
    hours: d.hours || "以当地公告为准",
    costType: d.costType || "free",
    facilities: d.facilities || {},
    seasonal: d.seasonal ?? false,
    featured: false,
    features: d.features || [],
    note: d.note || "具体开放以当地公告为准",
    ...extra,
  };
}

function mkDistrictGov(city, province, district, category, label, i) {
  const defaults = {
    gov_service: {
      costType: "free",
      hours: "工作日 9:00—17:00",
      facilities: { water: "partial", ac: true, wifi: "partial" },
      features: ["政务服务", "区县办事", "一窗受理"],
      note: "区县窗口业务以当地公告为准",
    },
    court: {
      costType: "free",
  ...
  const d = defaults[category] || {};
  return {
    id: stableId(city, category, `${district}-${label}-${i}`),
    name: `${city}${district}${label}`,
    category,
    city,
    province,
...
/** 浙江细粒度城市：按真实/模板区县加密政务与法院点位 */
function generateZhejiangGovExtras(cityMeta, province) {
  const city = cityMeta.name;
  if (!ZJ_DETAILED.has(city)) return [];

  const districts = city === "杭州" ? HZ_DISTRICTS : DISTRICT_SUFFIX;
  const govTypes = ["市民之家", "政务服务中心", "人社局办事大厅"];
  const courtTypes = ["区人民法院", "劳动人事争议仲裁院"];
  const out = [];

  districts.forEach((district, di) => {
    govTypes.forEach((label, ti) => {
      out.push(mkDistrictGov(city, province, district, "gov_service", label, di * 10 + ti));
    });
    courtTypes.forEach((label, ti) => {
      out.push(mkDistrictGov(city, province, district, "court", label, di * 10 + ti + 100));
    });
  });
  return out;
}

function generateCityAmenities(cityMeta, province) {
  const city = cityMeta.name;
  const specs = [
    [PARKING_LABELS, "parking"],
    [TOILET_LABELS, "toilet"],
    [CAMPING_LABELS, "camping"],
    [SPORTS_LABELS, "sports"],
    [CHARGING_LABELS, "charging"],
    [POLICY_LABELS, "policy"],
    [LEGAL_LABELS, "legal"],
    [TRAINING_LABELS, "training"],
    [GOV_SERVICE_LABELS, "gov_service"],
    [COURT_LABELS, "court"],
  ];
  const out = [];
  for (const [labels, category] of specs) {
    labels.forEach((label, i) => out.push(mkAmenity(city, province, category, label, i)));
  }
  out.push(...generateZhejiangGovExtras(cityMeta, province));
  return out;
}
...
function nationalTools() {
  const list = [
    ...
    { name: "全国公共露营信息", category: "camping", city: "全国", url: "https://www.mct.gov.cn/", features: ["露营基地", "户外休闲"], note: "文旅部门公开信息整理" },
    { name: "中国法律服务网", category: "legal", city: "全国", url: "https://www.12348.gov.cn/", features: ["法律咨询", "法律援助"], featured: true },
    { name: "国家职业技能培训", category: "training", city: "全国", url: "https://www.mohrss.gov.cn/", features: ["职业培训", "技能提升"] },
    { name: "国家政务服务平台 · 办事大厅", category: "gov_service", city: "全国", url: "https://www.gjzwfw.gov.cn/", features: ["政务服务", "跨省通办", "办事指南"], featured: true },
    { name: "中国法院网 · 诉讼服务", category: "court", city: "全国", url: "https://www.court.gov.cn/", features: ["诉讼服务", "立案指引", "司法公开"] },
  ];
  return list.map((s) => ({
    ...mkTool("全国", "全国", s),
    city: "全国",
...
function main() {
  ...
  for (const reg of REGIONS) {
    for (const cityMeta of reg.cities) {
      ...
      for (const r of generateCityAmenities(cityMeta, reg.province)) {
        if (seen.has(r.id)) continue;
        seen.add(r.id);
        resources.push(r);
      }
